    ...
    var EditorToolbarController = function () {
        ...
        _createClass(EditorToolbarController, [{
            ...
            value: function unpackTextFromInnerTag(parentTag, innerTag, el) {
                ...
                var trimContent = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;

                el.find(parentTag).each(function (_, el) {
                    var innerEl = el.querySelector(innerTag);
                    el.textContent = innerEl ? innerEl.textContent : fallbackText;
                    if (trimContent) {
                        el.textContent = el.textContent.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '');
                    }
                });
            }
        }, {
            ...
            key: 'tidyUpVideoModal',
            value: function tidyUpVideoModal(el) {
                var TAG = 'video-modal';
                this.unpackTextFromInnerTag(TAG, 'ng-transclude', el, '', true);
            }
        }, {
            ...
            value: function savePage() {
                ...
                var tidyUps = ['BeyerButton', 'BeyerIcon', 'BeyerCategoryProducts', 'BeyerAttributesComponent', 'PictureComponent', 'WidgetProducts', 'BeyerTileReference', 'TileAnimated', 'Carousel', 'VideoBg', 'VideoModal'];
