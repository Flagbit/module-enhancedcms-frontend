        ...
        _createClass(AbstractWidgetController, [{
            ...
            value: function formatWidgetHtml(html) {
                ...
                if (this.wrapWithLink(html)) {
                    ...
                    linkElement.setAttribute('fake-click', "location.href = '{[{data.widgetLink}]}'");
            ...
            value: function onMouseEnter(event) {
                ...
                // Hide all parent widgets' toolbars
                var target = event.currentTarget.parentElement;
                while (target && !(target.getAttribute('id') === 'ecms-builder' || (target.tagName || '').toLowerCase() === 'body')) {
                    if (target.classList.contains('ecms-widget-container-item')) {
                        angular.element(target).scope().$emit('hideToolbar');
                    }
            ...
            value: function onMouseLeave(event) {
                // Show direct parent widget's toolbar
                // No new mouseenter events are thrown if the element is "re-entered" from one of its own children,
                // so we have to help the parent out a bit.
                var target = event.currentTarget.parentElement;
                while (target && !(target.getAttribute('id') === 'ecms-builder' || (target.tagName || '').toLowerCase() === 'body')) {
                    if (target.classList.contains('ecms-widget')) {
                        angular.element(target).scope().$emit('showToolbar');
                        break;
                    }
                    target = target.parentElement;
                }
